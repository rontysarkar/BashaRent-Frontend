import { getTenantDashboardStats } from "@/actions/tenant/tenant.action"
import {  ClipboardList, Clock, Home, Wallet } from "lucide-react"

export default async function TenantDashboardPage() {
  const res = await getTenantDashboardStats()
  const result = res?.success ? res?.data : []
  return (
    <div className="p-6">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800">
          Dashboard Overview
        </h1>
        <p className="mt-1 text-sm text-slate-500">
          A quick summary of your request and activity
        </p>
      </div>

      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border bg-white p-8 shadow-sm transition hover:shadow-md">
          <div className="flex items-start justify-between">
            <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-blue-50">
              <ClipboardList size={28} className="text-blue-600" />
            </div>
          </div>
          <h2 className="mt-6 text-4xl font-bold text-slate-800">
            {result?.totalRequests ?? 0}
          </h2>
          <p className="mt-2 text-sm font-medium text-slate-500">
            Total Requests
          </p>
          <p className="mt-1 text-xs text-slate-400">
            All maintenance and service requests made.
          </p>
        </div>

        <div className="rounded-2xl border bg-white p-8 shadow-sm transition hover:shadow-md">
          <div className="flex items-start justify-between">
            <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-amber-50">
              <Home size={28} className="text-amber-600" />
            </div>
          </div>
          <h2 className="mt-6 text-4xl font-bold text-slate-800">
            {result?.activeRentals ?? 0}
          </h2>
          <p className="mt-2 text-sm font-medium text-slate-500">
            Active Rentals
          </p>
          <p className="mt-1 text-xs text-slate-400">
            Currently ongoing property rentals.
          </p>
        </div>
        <div className="rounded-2xl border bg-white p-8 shadow-sm transition hover:shadow-md">
          <div className="flex items-start justify-between">
            <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-amber-50">
              <Clock size={28} className="text-amber-600" />
            </div>
          </div>
          <h2 className="mt-6 text-4xl font-bold text-slate-800">
            {result?.pendingRequests ?? 0}
          </h2>
          <p className="mt-2 text-sm font-medium text-slate-500">
            Pending Requests
          </p>
          <p className="mt-1 text-xs text-slate-400">
            Requests waiting for owner approval.
          </p>
        </div>

        <div className="rounded-2xl border bg-white p-8 shadow-sm transition hover:shadow-md">
          <div className="flex items-start justify-between">
            <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-green-50">
              <Wallet size={28} className="text-green-600" />
            </div>
          </div>
          <h2 className="mt-6 text-4xl font-bold text-slate-800">
            ৳ {result?.totalPayments ?? 0}
          </h2>
          <p className="mt-2 text-sm font-medium text-slate-500">
            Total Payments
          </p>
          <p className="mt-1 text-xs text-slate-400">
            Cumulative amount paid so far.
          </p>
        </div>
      </div>
    </div>
  )
}
